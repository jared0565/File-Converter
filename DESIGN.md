...
The production logo combines a forest-green rounded square with opposing white/sage folded arrows and an outlined Inter SemiBold wordmark. Dark surfaces use the pale-sage tile and light lettering variant. Use the provided SVG assets rather than recreating the mark with text or filters. Header lockups are 216px wide on desktop and 170px on mobile; footer lockups are 158px. Preserve the 337:64 aspect ratio. The favicon uses only the symbol.

The footer uses three desktop columns for the brand, tool navigation and connection links, followed by a copyright/privacy row. Mobile layouts place the brand above two navigation columns and stack the bottom row. Copyright is attributed to Bytes Enterprises Ltd.; the LinkedIn link points to the owner-supplied profile and clearly announces that it opens a new tab. The year updates automatically in the browser.
